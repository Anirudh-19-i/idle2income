...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // If user session exists immediately, email confirmation is disabled → go to browse
    ...
    return (
      <main className="min-h-screen bg-orange-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-lg p-10 w-full max-w-md text-center">
          ...
          <h1 className="text-2xl font-bold text-orange-600 mb-2">Check your email!</h1>
          <p className="text-gray-500 text-sm mb-6">
            We sent a confirmation link to <strong>{form.email}</strong>.
            Please click it to activate your account, then come back to log in.
          </p>
          <Link href="/login" className="w-full block bg-orange-500 text-white py-3 rounded-lg font-semibold hover:bg-orange-600 text-center">
            Go to Login
          </Link>
        </div>
      </main>
